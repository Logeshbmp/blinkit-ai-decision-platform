...
-- 4. Aggregate Customer Sentiment
daily_feedback AS (
    SELECT 
...
SELECT 
    ...
    -- ROAS
    ROUND((COALESCE(o.total_revenue, 0) / NULLIF(m.total_spend, 0))::numeric, 2) AS roas,

    -- Operations
    ROUND(COALESCE(o.avg_delay_mins, 0)::numeric, 2) AS avg_delay,

    -- Profit
    ROUND(COALESCE(dp.total_profit, 0)::numeric, 2) AS estimated_profit,

    -- Customer Rating
    ROUND(COALESCE(f.avg_rating, 0)::numeric, 1) AS customer_satisfaction

FROM daily_marketing m
FULL OUTER JOIN daily_orders o 
    ON m.report_date = o.report_date

LEFT JOIN daily_profit dp 
